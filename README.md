...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`proc-macro2`](https://crates.io/crates/proc-macro2).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `proc-macro2` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Procedural-macro token model (decoupled from compiler)
...
Original work copyrighted by the upstream `proc-macro2` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
