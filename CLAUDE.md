# Claude Code Project Instructions — proc-macro2-kotlin

## Project Overview

This is **proc-macro2-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`proc-macro2`](https://crates.io/crates/proc-macro2).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/proc-macro2/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/proc-macro2>
